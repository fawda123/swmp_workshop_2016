...
$$\tau = \sum_{i = 1}^{n - 1}\sum_{j = i + 1}^{n} sign\left[\left(X_j - X_i\right)\left(Y_j - Y_i\right)\right]$$
\begin{center}
$\tau$ will vary from -1 to 1 similar to a correlation coefficient, follows an approximate normal-distribution for hypothesis-testing
\end{center}
\vfill
\onslide<2->
...
\begin{itemize}
\item<2-> Overall $\tau$ is the weighted average of the seasonal estimates \\~\\
